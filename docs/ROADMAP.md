...
| Settings redesign: everyday settings (AI brain/model, sync speed, price checker, voice, orb) up top full-size; Oracle setup + account linking in collapsible sections (open first-run, collapsed once set, live header status, never hidden) | ✅ CODE DONE 2026-07-12 — needs on-Windows visual check |
...
| W4-30 | **Scaling & BIS Advisor** — tag-graph reverse lookup → PoB weighted tree → BIS craft planner → Craft of Exile (spec: `docs/SCALING_ADVISOR_SPEC.md`) | ◑ P1+P1.5 SHIPPED 2026-07-12 | XL | P1 done: `core_engine/tag_graph.py` reverse-looks-up assets sharing an entity's real tags (idf-weighted), wired into the Orb's grounding as a "SCALING SOURCES (from DB)" block; 28 checks. P1.5 done 2026-07-12: per-page-kind tag extraction (uniques/base items via `KeywordPopups`, passives via keyword cards, mod tables via `badge[data-tag]`, gems unchanged) + `_page_kind` classifier + stored `knowledge_ledger.kind` (with migration); tag_graph groups by real kinds; 33 checks (`tests/scraper_tag_checks.py`). Selectors verified against the live poe2db DOM. Ledger converges as syncs re-tag pages — run a Sync to populate. P2.5 ENGINE done 2026-07-12: `core_engine/community_tags.py` (player-reported tags/relations, votes, verify-promotion) merged into tag_graph with "player-reported, unverified" marking + applier surfacing; 28 checks (`tests/community_tag_checks.py`); remaining: the "Suggest a tag" UI (on-Windows pass). Remaining: P2 PoB weighted tree, P3 BIS craft planner, P4 poe.ninja meta-mining. |
...
| W4-33 | **Kalandra Voice** — free offline neural TTS (Piper), zero-auth "Get free voice" button | ✅ v1 SHIPPED 2026-07-12 | M | `scripts/install_voice.py` (engine + curated voice menu → `tools/piper/`, atomic config, smoke test), `voice_engine` piper backend (voice_id `piper`, envelope/Rhubarb path unchanged, pyttsx3 fallback, winsound last resort), Settings picker entry + one-click download button, installer component (`extras`, default ON), `launchers/Install Kalandra Voice.bat`, Setup&Deps registry row. Default voice: en_GB-alan-medium; swap via `--voice` (menu: `--list`; previews: https://rhasspy.github.io/piper-samples/). ElevenLabs stays the future premium opt-in. |
| W4-32 | **Community Quest Board** — crowdsourced data upkeep, verify-before-close lifecycle (spec: `docs/COMMUNITY_QUEST_BOARD_SPEC.md`) | ◑ P1 ENGINE SHIPPED 2026-07-12 | L | `core_engine/quest_board.py`: TagQuests auto-generated from untagged ledger rows + maintainer-posted quests; `open→claimed→submitted→verified→closed` enforced structurally (close only from verified; threshold of DISTINCT agreeing contributors or maintainer sign-off; gap must actually be gone; `recheck_gaps` reopens on regression); verified resolutions write into the community tag layer pre-verified; contributor credits. 33 checks (`tests/quest_board_checks.py`). Remaining: the dashboard "Quest Board" pane (on-Windows pass), P3 auto-quests from nerf_intel/meta-mining, P4 shared board. |
