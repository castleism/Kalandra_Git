...
2. Double-click **`Install Dependencies.bat`** (or run
   `py -3.12 install_dependencies.py`). You'll get a menu:
   ...
   ```
   py -3.12 install_dependencies.py --all       # everything, no prompts
   py -3.12 install_dependencies.py --minimal   # Python packages only
   py -3.12 install_dependencies.py --yes core extras   # named groups
   ```
...
python main.py
```
or double-click **`Windows Diagnostic Launcher.bat`** (keeps the console open so
...
- See `SECURITY_AND_PRIVACY.md`, `ROADMAP.md`, and the other `*_SETUP.md` docs
  for details.
