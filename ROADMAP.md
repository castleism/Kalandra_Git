...
`SECURITY_AND_PRIVACY.md`, the feature roadmap, and the routes we considered
...
| Your own (non-ladder) PoE2 characters via API | ☐ Blocked on GGG shipping the PoE2 OAuth API (letter drafted: `GGG_OAuth_Application_Letter.md`) |
| Higher-fidelity face rig (layered art, blinks) | ☐ Optional — drop in layered mouth/eye art (`VISEME_FACE_SETUP.md`) |
