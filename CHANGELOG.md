...
- **New `core_engine/quest_board.py`** — the local quest board. Knowledge-base
  ...
  `generate_from_db` spawns a *TagQuest* for every untagged `knowledge_ledger`
  row (no duplicates; bounded per run), and maintainers can post
  tag / investigate / verify / relationship quests manually.
- **Lifecycle enforced structurally:** `open → claimed → submitted → verified
  → closed` (+ `reopened`). `closed` is unreachable except from `verified`;
  `verified` requires ≥ N *distinct* contributors submitting the same
  (normalized) proposal — one player re-submitting never counts twice — or a
  trusted maintainer's sign-off. Gap-generated quests additionally refuse to
  close until the gap is actually gone (scraped tags present or verified
  community tags), and `recheck_gaps` re-opens closed quests whose gap
  returns after a scrape/patch. Claims are soft, non-exclusive, and expire.
- **Wired to the community tag layer** (early P2 slice): a verified
  tag/relationship resolution is written into `community_tags.py`
  pre-verified, so the Scaling Advisor uses it immediately; contributor
  credits are recorded per verified quest (the gamified hook).
- Local-only JSON overlay (ship-safe, survives DB rebuilds). ToS: quests are
  about metadata, never actions in the game client. Remaining for P1: the
  dashboard "Quest Board" pane (needs the on-Windows pass).
- Tests: `tests/quest_board_checks.py` (33 checks) — generation, dedup, the
  full state machine, can't-close-without-verify, gap-gone closing,
  reopen-on-regap, threshold vs sign-off, write-back, persistence.

### Added — 2026-07-12 Scaling Advisor P2.5 (engine): player-reported tags & interactions (W4-30)
- **New `core_engine/community_tags.py`** — a JSON overlay (the proven
  `knowledge_corrections.py` pattern: ship-safe, survives DB rebuilds) where
  the player proposes tags ("Widowhail: Bow") and relationships ("The
  Peacemaker's Draught **applies** Corrupted Blood" / "affected_by"). Entries
  carry `source="community"`, votes (re-suggesting = +1 vote, no duplicates),
  and a `verified` flag with a maintainer promotion path — mirroring verified
  corrections. Local-only by default (abuse guard); ToS unaffected — metadata
  about the game, never interaction with it.
- **Merged into the tag graph, visibly marked:** community tags widen the
  reverse lookup (an untagged new unique becomes findable the moment the
  player tags it) but are returned separately and rendered as
  "player-reported, UNVERIFIED — present as community hints, never as fact";
  maintainer-verified ones get their own label. "Applied by (player-reported)"
  line surfaces community appliers — the future P4 meta-miner's applier list.
  With an empty overlay, every result is byte-identical to before.
- Remaining for P2.5: the "Suggest a tag / interaction" UI action (needs the
  on-Windows pass; the engine API is ready for it).
- Tests: `tests/community_tag_checks.py` (28 checks) — dedup-as-vote,
  promotion, persistence, overlay merge/marking, applier surfacing,
  no-behavior-change with an empty overlay.

### Changed — 2026-07-12 Settings menu redesign: everyday settings up top, setup folds away
- **Settings & Accounts is no longer a wall.** The everyday settings — AI
  brain + model, DB sync speed, price checker, Divine Orb voice, companion
  orb — sit at the top at full size. The set-once stuff moved into two
  themed **collapsible sections**: 🔮 *Oracle setup* (API key, session
  budget, usage) and 🔗 *Account linking* (builds / tools / external
  accounts).
- **Open on first run, collapsed once set, never hidden:** each section
  starts expanded until it's configured (a saved key; at least one linked
  account), then future opens start folded — with a live status on the
  header (🟢 key saved / 🟢 3 linked) so a closed section still tells you
  its state. One click reopens; a manual toggle is never overridden
  mid-session. Dashboard-tab and custom-tab pickers stay always visible.
- New `CollapsibleSection` widget (gui_overlay/mirror_window.py) reusable for
  future dialogs.
- Tests: `tests/settings_layout_checks.py` (30 source-level checks — layout
  structure, collapsed-once-set wiring, nothing-hidden guarantees, and an
  unbound-name AST audit for the new widget, since the sandbox can't render
  Qt). **Needs an on-Windows visual check.**

### Added — 2026-07-12 Scaling Advisor P1.5: real tags for uniques, mods & passives (W4-30)
- **The tag graph is no longer gem-only.** `scraper._extract_tags` now reads
  each page kind's own tag markup — all selectors verified against the live
  poe2db DOM in a real browser before writing a line of extractor:
  - **Uniques & base items** — `<a class="KeywordPopups">` keyword chips inside
    the item popup (each carries a canonical `data-keyword`; camel-case ids
    like `DamageOverTime` are split to "Damage Over Time" so they share the
    gem-tag vocabulary).
  - **Passive nodes** (keystones + notables) — the same keyword links from the
    node's keyword card; an *embedded* related-gem popup on the page is
    explicitly skipped so its tags never bleed onto the node.
  - **Mod tables** (item-class pages like `/us/Amulets`) — the
    `span.badge[data-tag]` chips on the ModifiersCalc rows (Life, Mana, Fire…).
  - **Gems** — unchanged (`a.GemTags`), no regression.
- **New page-kind classifier** (`scraper._page_kind`): reads the page's first
  `newItemPopup` class (`UniquePopup` / `keystonePopup` / `notablePopup` /
  `GemPopup` / `NormalPopup`…) with a mod-table check before the base-item
  fallback; gem pages are refined to `skill_gem` vs `support_gem` by their
  Support tag. Unknown stays `""` — a kind is never fabricated.
- **`knowledge_ledger.kind` column** (+ migration for existing DBs); the
  crawler stores the classification on insert *and* on re-crawl refresh, so the
  whole ledger converges to real kinds as syncs rotate through the site.
- **`tag_graph` now groups by the stored kind** (markup-verified) and only
  falls back to the old tags/URL inference for rows crawled before the column
  existed. New "Base items" group. The Scaling Advisor's "SCALING SOURCES"
  block therefore starts naming real uniques, craftable-mod pages, and passive
  nodes — not just gems — as the next Sync re-tags pages.
- Tests: `tests/scraper_tag_checks.py` (33 checks) — per-kind extraction and
  classification against fixtures modeled on the verified live markup, the
  kind-column migration, insert/update round-trip, back-compat calls, and
  tag-graph grouping via stored kinds with legacy-row fallback.

### Added — 2026-07-11 Scaling Advisor P1: tag-graph reverse lookup (W4-30)
- **The Orb now answers "what scales X?" from the database, not guesswork.**
  New `core_engine/tag_graph.py`: look an entity up → read its real poe2db tags →
  reverse-look-up every knowledge-base asset that shares those tags, ranked by
  weighted tag overlap (rare shared tags — via an idf weighting — count more than
  ubiquitous ones like `Physical`). `tags_for`, `assets_with_tags`,
  `entities_in_text`, `scaling_sources`, and a prompt-ready `context_block`.
- **Wired into the Orb's grounding** (`_ai_context`): a "SCALING SOURCES (from the
  DB)" block is injected for scaling/gearing questions, listing the real
  uniques / support gems / mods / passives that share the subject's tags, with an
  explicit instruction to name only assets that appear in it (anti-fabrication).
- Degrades honestly: if the entity is untagged or nothing shares its tags yet, it
  says so rather than inventing. Coverage grows as the crawler tags more page
  kinds (the reverse index reads `knowledge_ledger.tags`, added 2026-07-11).
- Tests: `tests/tag_graph_checks.py` (28 checks) — reverse-lookup ranking,
  overlap/idf weighting, kind inference, honest-degrade, and the
  name-only-real-assets contract. Spec: `docs/SCALING_ADVISOR_SPEC.md`.

### Added — 2026-07-11 Crawl drains to completion (W4-31)
- Sync now re-queues pages skipped due to timeouts/rate-limits and re-crawls
  until nothing retryable remains (`scraper.crawl_until_drained`), bounded by a
  25-pass cap and a stall guard so it always terminates. 404/410 pages are marked
  `dead` and never retried. On by default (`crawl_drain`); the sync worker uses
  it automatically.
